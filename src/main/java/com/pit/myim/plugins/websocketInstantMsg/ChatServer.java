package com.pit.myim.plugins.websocketInstantMsg;

import com.pit.myim.entity.PageData;
import com.pit.myim.service.*;
import com.pit.myim.util.Const;
import com.pit.myim.util.DateUtil;
import com.pit.myim.util.SpringUtil;
...
public class ChatServer extends WebSocketServer {
    ...
    public void onMessage(WebSocket conn, String message) {
       ...
       // log.debug("收到来之[{}]的消息[{}]", conn.getRemoteSocketAddress(), message);
        if (null != message && message.startsWith(",fhadmin-msg,")) {                    //用户发送的数据消息
         ...
         //   log.debug("[{}]用户上线加入群聊===ChatServer", conn.getRemoteSocketAddress());
    ...
    public void userjoin(String user, WebSocket conn) {
        ...
        try {
            //修改状态为在线
            //    pd.put(Const.SESSION_U_ONLINE, "onlie");
            //    imstateService.editOnline(pd);
            PageData ipd = new PageData();
            ipd = imstateService.findByUsername(pd);
            if (null != ipd && "online".equalsIgnoreCase(ipd.getString("online"))) {                    //只有用户设置的状态为在线的时候，才会向别人发送上线消息
                JSONObject result = new JSONObject();
    ...
    public void userLeave(WebSocket conn) {
        ...
        log.info("[{}]用户下线", conn.getRemoteSocketAddress());
    ...
    public void agreefriend(String message, WebSocket conn) {
        ...
        try {
            ...
            result.element("groupid", pd.getString("fgroupId"));                                                        //分组ID
